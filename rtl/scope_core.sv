...
#(
    ...
    // status
    output logic [2:0]            state,         // scope_state_e encoding (STATUS.state)
  ...
  wire capture_wr = sample_valid && (st == SCOPE_ST_FILLING || st == SCOPE_ST_ARMED ||
                                     (st == SCOPE_ST_TRIGGERED && post_remaining != '0));
  wire trig_accept = trig && sample_valid && (st == SCOPE_ST_ARMED);

  wire fill_wr = capture_wr && (st == SCOPE_ST_FILLING);
  wire post_wr = capture_wr && (st == SCOPE_ST_TRIGGERED);
    ...
    if (rst) begin
      ...
      if (disarm) begin
        ...
        unique case (st)
          ...
          SCOPE_ST_FILLING: begin
            if (fill_next >= {1'b0, pretrig}) st <= SCOPE_ST_ARMED;
          end
          SCOPE_ST_ARMED: begin
            if (trig_accept) st <= SCOPE_ST_TRIGGERED;
          end
          SCOPE_ST_TRIGGERED: begin
          ...
          default: st <= SCOPE_ST_IDLE;  // unreachable encodings recover to IDLE
        endcase
      end
    end
  end
  ...
  assign armed = (st == SCOPE_ST_FILLING) || (st == SCOPE_ST_ARMED);
