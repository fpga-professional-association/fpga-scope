...
// ===========================================================================================
// fpga-scope #12 board demo. PROVENANCE: this is ../../../hyperram/fpga/axc3000/top.sv with an
// fpga-scope instance added as a THIRD JTAG-Avalon slave (base 0x400) that watches the HyperRAM
// controller's bring-up signals. The hyperram datapath is untouched; the scope only taps the
// already-registered fabric probe nets (cap_*_q / *_dbg_state / av_*) and reads its CSR over the
// existing JTAG-Avalon master. Transport = XPORT="CSR" (zero extra pins). See fpga/axc3000/README.md.
// ===========================================================================================
module top_scope (
    // ---- board clock / reset ----
  ...
  // fpga-scope CSR (native bus, XPORT="CSR"): 256 word registers => 8 word-address bits =
  // m_address[9:2]. Combinational readdata (0 wait states), so it drops into the same mux.
  wire [7:0]        scope_csr_addr = m_address[9:2];
  wire              scope_csr_read = m_read  & sel_scope;
  ...
  // =========================================================================
  // fpga-scope #12: the production embedded logic analyzer, watching the same HyperRAM
  // bring-up nets as u_cap. 32-bit probe registered ONCE (single `clk` stage) so every lane
  // is coherent; scope_core samples it in the `clk` domain (no clk2x crossing, unlike u_cap).
  // Bit map is fpga/axc3000/probes.json (consumed by the #10 host tool for VCD/sigrok export).
  // =========================================================================
  logic [31:0] scope_probe_q;
  always_ff @(posedge clk) begin
    scope_probe_q <= {
      phy_dq_o_w[7:0],       // [31:24] TX byte A (fabric word, hi byte)
      ctrl_dbg_seg[5:0],     // [23:18] controller segment counter
      av_readdatavalid,      // [17]
      av_waitrequest,        // [16]
      av_write,              // [15]
      av_read,               // [14]
      ctrl_dbg_state,        // [13:10] controller FSM state
      fe_dbg_state,          // [9:8]   front-end FSM state
      phy_rst_n_w,           // [7]
      phy_rd_arm_w,          // [6]
      hb_rwds,               // [5] raw RWDS pad input
      phy_rwds_o_w[1],       // [4] 1st-phase write mask
      phy_rwds_oe_w,         // [3]
      phy_dq_oe_w,           // [2]
      phy_ck_en_w,           // [1] CK-enable proxy (pad CK unprobeable)
      phy_cs_n_w             // [0] chip select (falling edge = transaction start)
    };
  end

  scope_top #(
      .PROBE_W    (32),
      .DEPTH_LOG2 (12),      // 4096 words
      .NUM_CMP    (4),
      .SEQ_STAGES (4),
      .RLE_EN     (1'b1),    // demo the run-length store path (issue #9)
      .TS_W       (48),
      .XPORT      ("CSR"),   // CSR-over-JTAG-Avalon: zero extra board pins
      .ID_VALUE   (32'hA3C3_0012)
  ) u_scope (
      .clk          (clk),
      .rst          (sys_rst),
      .probe        (scope_probe_q),
      .trig_ext_i   (1'b0),
      .trig_ext_o   (),                 // no cross-instance loop
      .xclk         (clk),              // CSR mode: byte transport unused (tied off in scope_top)
      .xrst         (sys_rst),
      .rx_data      (8'h0),
      .rx_valid     (1'b0),
      .rx_ready     (),
      ...
      .armed        (),
      .triggered    (),
      .ext_csr_addr (scope_csr_addr),
      .ext_csr_wdata(m_writedata),
